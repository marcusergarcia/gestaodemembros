...
import { getGruposRef, getGrupoDocRef, getMembroDocRef } from "@/lib/firestore-helpers";
...
export default function GruposPage() {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        if (grupo.liderMembroId) {
          const liderDoc = await getDoc(getMembroDocRef(igrejaId, grupo.liderMembroId));
          if (liderDoc.exists()) {
        ...
        for (const memberId of grupo.membrosIds.slice(0, 3)) {
          const memberDoc = await getDoc(getMembroDocRef(igrejaId, memberId));
          if (memberDoc.exists()) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await updateDoc(getGrupoDocRef(igrejaId, grupoToDelete.id), {
        ativo: false,
      });
